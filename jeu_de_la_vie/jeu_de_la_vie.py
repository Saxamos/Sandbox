...
def click_gauche(event):  # fonction rendant vivante la cellule cliquÃ©e donc met la valeur 1 pour la cellule
    x = event.x - (event.x % c)
    y = event.y - (event.y % c)
...
def click_droit(event):  # fonction tuant la cellule cliquÃ©e donc met la valeur 0 pour la cellule cliquÃ©e au dico_case
    x = event.x - (event.x % c)
    y = event.y - (event.y % c)
...
def canon():  # fonction dessinant le cÃ©lÃ¨bre canon Ã  planeur de Bill Gosper
    dico_case[0 * c, 5 * c] = 1
    dico_case[0 * c, 6 * c] = 1
    dico_case[1 * c, 5 * c] = 1
    dico_case[1 * c, 6 * c] = 1
...
def play():  # fonction comptant le nombre de cellules vivantes autour de chaque cellule
    ...
    while v != width / c:
        ...
        while w != height / c:
            ...
            # cas spÃ©ciaux:
            # les coins
            if x == 0 and y == 0:  # coin en haut Ã  gauche
                compt_viv = 0
            ...
            elif x == 0 and y == int(height - c):  # coin en bas Ã  gauche
                compt_viv = 0
                if dico_case[x, y - c] == 1:
                    compt_viv += 1
                if dico_case[x + c, y - c] == 1:
                ...
                if dico_case[x + c, y] == 1:
                    compt_viv += 1
                dico_etat[x, y] = compt_viv
            elif x == int(width - c) and y == 0:  # coin en haut Ã  droite
                compt_viv = 0
                if dico_case[x - c, y] == 1:
                    compt_viv += 1
                if dico_case[x - c, y + c] == 1:
                ...
                if dico_case[x, y + c] == 1:
                    compt_viv += 1
                dico_etat[x, y] = compt_viv
            elif x == int(width - c) and y == int(height - c):  # coin en bas Ã  droite
                compt_viv = 0
                if dico_case[x - c, y - c] == 1:
                    compt_viv += 1
                if dico_case[x - c, y] == 1:
...
def redessiner():  # fonction redessinant le tableau Ã  partir de dico_etat
    can1.delete(ALL)
    damier()
    t = 0
    while t != width / c:
...
# vitesse de l'animation (en rÃ©alitÃ© c'est l'attente entre chaque Ã©tapes en ms)
vitesse = 50

flag = 0
dico_etat = {}  # dictionnaire contenant le nombre de cellules vivantes autour de chaque cellule
dico_case = {}  # dictionnaire contenant les coordonnÃ©es de chaques cellules et une valeur 0 ou 1 si elles sont
# respectivement mortes ou vivantes
i = 0
while i != width / c:  # assigne une valeur 0(morte) a chaque coordonnÃ©es(cellules) (valeur par dÃ©fault en quelque
    # sorte ^^)
    j = 0
    while j != height / c:
        x = i * c
